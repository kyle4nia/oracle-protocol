// Oracle Protocol — GENESIS ESTABLISHING tx.
// Spends a plain P2PK wallet UTXO (signed with our hex key) and creates the
// rep=100 covenant UTXO at the covenant P2SH address, binding it via
// populate_genesis_covenants so it carries the correct covenant_id.
//
// This is the step that was missing: a covenant UTXO must be CREATED as a
// bound output of a signed tx, not funded directly at the P2SH address.
//
...
// Plain P2PK wallet address we control (key below).
...
// rep=100 redeem script (the covenant). The genesis output pays to its P2SH.
...
async fn main() {
    ...
    println!("[genesis] connecting to grpc://{NODE}");
    ...
    if resp.is_empty() {
        println!("[genesis] no UTXO on funding address. aborting.");
        return;
    }
    let entry = &resp[0];
    ...
    println!("[genesis] funding input: {}:{}  amount {} sompi", outpoint.transaction_id, outpoint.index, in_amount);
    ...
    // The genesis covenant output: pays to the rep=100 covenant P2SH, carries the value.
    let covenant_spk = pay_to_script_hash_script(&REDEEM_REP100.to_vec());
    ...
    println!("[genesis] genesis covenant_id bound to output: {bound_cid:?}");
    println!("[genesis] covenant output value: {out_value} sompi (fee {FEE})");
    ...
    println!("[genesis] built+signed tx id: {}", tx.id());

    if !go {
        println!();
        println!("[genesis] DRY RUN — not submitted. Re-run with --go to broadcast.");
        return;
    }

    println!("[genesis] SUBMITTING...");
    match rpc.submit_transaction((&tx).into(), false).await {
        Ok(resp) => println!("[genesis] ACCEPTED. node response: {resp:?}"),
        Err(e) => println!("[genesis] REJECTED: {e}"),
    }
}
